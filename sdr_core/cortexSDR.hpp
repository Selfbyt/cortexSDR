/**
 * @file cortexSDR.hpp
 * @brief Core header for sparse distributed representation (SDR) functionality
 * 
 * This header defines the main SparseDistributedRepresentation class and supporting
 * structures that provide brain-inspired sparse encoding for various data modalities.
 * The implementation uses neural network principles to create efficient, sparse
 * representations suitable for compression and pattern recognition.
 * 
 * Key Features:
 * - Multi-modal encoding (text, numeric, image, audio, video, geographic)
 * - Brain-inspired sparse distributed representations
 * - Configurable sparsity levels and encoding parameters
 * - Pattern-based compression with vocabulary management
...
/**
 * @brief Encoding range definitions for different data modalities
 * 
 * Defines the sparse vector index ranges allocated to different types
 * of encoders to prevent conflicts and enable efficient encoding.
 */
struct EncodingRanges {
    static constexpr size_t WORD_START = 0;                    ///< Word encoding start index
    static constexpr size_t WORD_END = 9999;                   ///< Word encoding end index
    static constexpr size_t SPECIAL_CHAR_START = 10000;        ///< Special character encoding start
