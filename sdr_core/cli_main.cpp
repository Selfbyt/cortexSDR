...
/**
 * @file cli_main.cpp
 * @brief Command-line interface for CortexSDR compression and decompression
 * 
 * This utility provides command-line access to CortexSDR's sparse distributed
 * representation (SDR) compression capabilities for text files.
 * 
 * Features:
 * - Text compression using sparse distributed representations
 * - Configurable sparsity levels for compression ratio tuning
 * - Optional run-length encoding (RLE) for maximum compression
 * - Gzip compression support for additional size reduction
 * - Bit-packed SDR format for storage efficiency
 * - Delta-encoded vocabulary compression
 * 
 * Usage:
 * - Compress: ./cli_main -c input.txt output.sdr
 * - Decompress: ./cli_main -d input.sdr output.txt
 * - With options: ./cli_main -c --sparsity 0.001 --rle input.txt output.sdr.gz
 */
...
/**
 * @brief Serialize SDR data and vocabulary to compressed file format
 * @param filePath Output file path for the serialized data
 * @param sdr SparseDistributedRepresentation object containing data and vocabulary
 * @throws std::runtime_error if file operations fail
 * 
 * File format:
 * - Version byte (1 = bit-packed format)
 * - Total SDR size (32-bit)
 * - Bit-packed active positions
 * - Vocabulary size (64-bit)
 * - Delta-encoded vocabulary entries
 */
...
/**
 * @brief Deserialize SDR data and vocabulary from compressed file
 * @param filePath Input file path containing serialized data
 * @param sdr SparseDistributedRepresentation object to populate
 * @throws std::runtime_error if file operations or format validation fails
 * 
 * Supports both current bit-packed format (version 1) and legacy format
 * for backwards compatibility with older compressed files.
 */
 ...
 * @param progName Program name for usage display
 * 
 * Displays comprehensive usage instructions including all available
 * options, compression modes, and example command lines.
 */
