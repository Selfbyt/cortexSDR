/**
 * @file NumberEncoding.cpp
 * @brief Implementation of numerical data sparse distributed representation encoding
 * 
 * This file implements the NumberEncoding class which provides intelligent
 * encoding of numerical data using sparse distributed representations with
 * adaptive bucketing strategies and statistical analysis.
 * 
 * Key Features:
 * - Adaptive bucketing for optimal number distribution
 * - Statistical analysis for data-driven encoding parameters
 * - Special handling for NaN, infinity, and edge cases
 * - Configurable precision and sparsity levels
 * - Memory-efficient sparse representation generation
 * - Support for wide range of numerical data types
 */
...
// Special value markers for edge cases (using reserved index space)
static constexpr size_t RANGE_START_MARKER = std::numeric_limits<size_t>::max() - 1;  ///< Marker for range boundaries
static constexpr size_t NAN_MARKER = std::numeric_limits<size_t>::max() - 2;          ///< Marker for NaN values
static constexpr size_t INF_POS_MARKER = std::numeric_limits<size_t>::max() - 3;      ///< Marker for +infinity
static constexpr size_t INF_NEG_MARKER = std::numeric_limits<size_t>::max() - 4;      ///< Marker for -infinity

/**
 * @brief Constructor for NumberEncoding with configuration parameters
 * @param config Configuration structure containing encoding parameters
 * 
 * Initializes the number encoding system with specified parameters including
 * bucket count, sparsity levels, and statistical analysis settings.
 */
